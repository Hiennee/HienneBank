...
app.get("/users/:username", async (req, res) => {
    res.status(234).send(await userCol.findOne({ username: req.params.username.toUpperCase() }));
})
...
app.put("/update/users/username/:username/", async (req, res) => 
{
    ...
    if (await userCol.findOne({ username: newUserName }) == null) {
        ...
        console.log(`Updated user ${req.params.username.toUpperCase()} to ${newUserName}`)
        return;
    }
    ...
    // var result = await (await db).query(`UPDATE USER SET username = '${newUserName}' where username = '${req.params.username.toUpperCase()}'`)
    // res.status(234).send({ message: `Updated user ${req.params.username.toUpperCase()} to ${newUserName}` })
    // console.log(`Updated user ${req.params.username.toUpperCase()} to ${newUserName}`)
})
...
app.put("/update/users/password/:username", async (req, res) => 
{
    ...
    console.log(`Updated user ${req.params.username.toUpperCase()}'s password to ${newPassword}`)
    ...
    // var result = await (await db).query(`UPDATE USER SET password = '${newPassword}' where username = '${req.params.username.toUpperCase()}'`)
    // res.status(234).send({message: `Updated user ${req.params.username.toUpperCase()}'s password to ${newPassword}`})
    // console.log(`Updated user ${req.params.username.toUpperCase()}'s password to ${newPassword}`)
})
...
app.put("/update/users/banknum/:username/", async (req, res) => 
{
    ...
    if (await userCol.findOne({ banknum: newBanknum }) == null) {
        ...
        console.log(`Updated ${req.params.username.toUpperCase()}'s bank number to ${newBanknum} successfully`)
        return;
    }
    ...
    //     return;
    // }
    // var result = await (await db).query(`UPDATE USER SET banknum = '${newBanknum}' where username = '${req.params.username.toUpperCase()}'`)
    // res.status(234).send({message: `Updated user ${req.params.username.toUpperCase()}'s bank number to ${newBanknum}`})
    // console.log(`Updated user ${req.params.username.toUpperCase()}'s bank number to ${newBanknum}`)
})
...
app.put("/update/users/phonenum/:username/", async (req, res) => 
{
    ...
    if (await userCol.findOne({ phonenum: newPhonenum }) == null) {
        ...
        console.log(`Updated ${req.params.username.toUpperCase()}'s phone number to ${newPhonenum} successfully`)
        return;
    }
...
app.get("/money/:username", async (req, res) =>
{
    ...
    if (result == null) {
        console.log("Error getting money from user " + req.params.username.toUpperCase());
        res.status(345).send({ message: "Error getting money from user " + req.params.username.toUpperCase() });
        return
    }
    res.status(234).send({ money: result.money });
    console.log("Money:", result.money);
    // var result = await (await db).query(`SELECT money FROM USER where username = '${req.params.username.toUpperCase()}'`);
    // if (result[0].length == 0) {
    //     console.log("Error getting money from user " + req.params.username.toUpperCase());
    //     res.status(345).send({ message: "Error getting money from user " + req.params.username.toUpperCase() });
    //     return
...
{
    var result = await moneyHistoryCol.find({ username: req.params.username.toUpperCase() }).toArray();
    res.status(234).send(result);
    console.log(result);
    // var result = await (await db).query(`SELECT * FROM MoneyHistory WHERE username = '${req.params.username.toUpperCase()}'`);
    // var listData = []
    // result[0].forEach((r) => {
    //     listData.push(r)
    //     console.log(r)
